...
export const Carousel = ({sliderBlogs}) => {
  ...
  return (
     <div className='w-full h-[60rem]  mt-[2rem]'>
          ...
          <svg className="w-8 h-8 " fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
            ...
            {sliderBlogs.map((blog,i)=>(
                ...
                <div className='flex flex-col h-[70%] absolute top-[25%] left-[15%] w-[50%]'>
                    <p className='text-[white] text-[1.5rem] '>July 13, 2022 <span className='mx-[.5rem]'>|</span>Admin</p>
                    <h3 className='text-[3.5rem] text-[#ff8400] w-[70%]'>{blog.title}</h3>
            ...
            <div className='absolute bottom-5 left-1/2'>
              {sliderBlogs.map((image,i)=>(
                <button onClick={()=>SlideBtn(i)} className={`h-4 w-4 rounded-full ${curSlide===i?'bg-gray-700 border-2 border-white':'border-2 border-white'} bg-white m-2`}></button>
              ))}
            </div>     
