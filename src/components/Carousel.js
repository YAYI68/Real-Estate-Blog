        ...
        <button  onClick={moveRight}  className={`absolute right-[1%] top-[50%] text-[2.5rem] z-10 translate-y-[-50%] h-[4rem] w-[4rem] rounded-full bg-[#d5d1d1] flex items-center justify-center`}>
          <svg className="w-8 h-8 " fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
            ...
            {sliderBlogs.map((blog,i)=>(
                ...
                <img src={blog.imageURL} alt="" className=" h-full w-full brightness-[.5] object-top" />
                <div className='flex flex-col h-[70%] absolute top-[25%] left-[15%] w-[50%]'>
                    ...
                    <h3 className='text-[3rem] text-[#ff8400] w-[70%]'>{blog.title}</h3>
                    ...
                    <Link to="" className='text-[white] text-[1.2rem] px-[2rem] py-[1rem] bg-[#8034eb] w-fit text-center my-[1.5rem] rounded' >Read More</Link>
                </div>
              </div>    
